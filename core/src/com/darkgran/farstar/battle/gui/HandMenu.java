...
public class HandMenu extends TokenMenu {
    ...
    public void generateTokens() {
        ...
        for (int i = 0; i < getCardList().getCards().size(); i++) {
            getTokens().add(new HandCard(getCardList().getCards().get(i), getX() + getOffset()*i, getY()));
        }
    }

    @Override
    public void generateNewToken(Card card) {
        getTokens().add(new HandCard(card, getX() + getOffset()*getTokens().size()-1, getY()));
    }
